    ...
    if request.session['type'] == 'faculty':
        ...
        if request.method == 'POST':
            if 'search' in request.POST:
                ...
                students = student_profile.objects.filter(coursetaken=selected_course,
                                                          Semester=selected_course.semester).order_by('user')
                return render(request, 'attendance_form.html',
                              {'courses': courses, 'students': students, 'selected_course': selected_course})
                ...
                if Attendance.objects.filter(course=selected_course, date=now):
                    return render(request, 'attendance_form.html',
                                  {'courses': courses, 'error': "You have already submitted today's attendance"})
                else:
                    ...
                    for student in present_students:
                        ...
                        att.present.add(s)
                    return render(request, 'attendance_form.html',
                                  {'courses': courses, 'success': "Attendance has been submitted successfully"})
    ...
    if request.method == 'POST':
        if 'search' in request.POST:
            ...
            return render(request, 'attendance_result.html',
                          {'temp': temp, 'courses': courses, 'students': students, 'selected_course': selected_course,
                           'total': total})
